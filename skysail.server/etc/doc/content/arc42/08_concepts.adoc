...
.Motivation
Some concept cannot be â€œfactoredâ€� into a separate building block of the architecture (e.g. the topic â€œsecurityâ€�). This section of the template is the location where you can describe all decision for such a cross cutting topic in one central place. Nevertheless, you have to make sure that all your building blocks conform to such decisions.
...
Often these structures or concepts are re-used at many places within an architecture, especially within chapter [Building Block View]â€¦

.Motivation
Often parts of these models reappear in the building block view, but in order to â€žkeep them clean from technnologyâ€œ  they can be captured as a concept.
...
[role="arc42help"]
****
*Communication*: Exchange of data between system components. Covers communications within one process or address space, between different processes (inter-process communication â€“ IPC), and between different systems.
...
[role="arc42help"]
****
**Distribution**: Design of software systems whose parts are executed on different â€“ physically separated â€“ hardware systems.

Distribution covers issues such as calling methods on remote systems (remote procedure call â€“ RPC or remote method invocation â€“ RMI), the transfer of data or documents among distributed parties, the choice of optimal modes of interaction or communications patterns (such as synchronous / asynchronous, publish-subscribe, peer-to-peer).
****
...
[role="arc42help"]
****
Larger software systems are often executed in controlled environments (data centers) under oversight of operators or administrators. These stakeholders require specific information on the applicationsâ€™ states during runtime as well as special means of control and configuration.
****
...
[role="arc42help"]
****
There are two ways of documenting an applicationâ€™s status during runtime: *Logging* and *Tracing*. In both cases the application is extended with function or method calls that write state information, but there is a difference in their usage:
...
* During development: For example server, file, or directory names could be stored directly in the code (â€œhard-codedâ€�).
...
[role="arc42help"]
****
This section covers support for usage of the system in different countries, i.e. adjusting the system to country specific attributes. Internationalization (often abbreviated as â€œi18nâ€� where â€œ18â€� refers to the eighteen characters between the I and the n) covers translation of text, usage of character encodings, display of fonts, writing of numbers and dates, and other (external) aspects.
****
...
[role="arc42help"]
****
Support for simple (and if possible automated) tests. This aspect is the basis for the important implementation pattern of â€œcontinuous integrationâ€�. Projects should support at least daily build-and-test cycles. Important keywords for this aspect are unit tests and mock objects.
****
...
[role="arc42help"]
****
How and where do you use code generators to create parts of the system from models or domain specific languages (DSLâ€™s)?
****
