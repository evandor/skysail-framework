            ...
            if (type.isArray() && (index < 0)) { // Scalar value into array
                ...
                if (value instanceof String && Collection.class.isAssignableFrom(type) ) {
                    ...
                    try {
                        ...
                        Class<?> parameterizedType = ReflectionUtils.getParameterizedType(fieldAnnotation.repository());
                        if (Identifiable.class.isAssignableFrom(parameterizedType)) {
                            List<String> ids = Arrays.stream(((String)value).split(",")).map(String::trim).collect(Collectors.toList());
                            for (String id : ids) {
                                Identifiable newInstance;
                                try {
                                    newInstance = (Identifiable) parameterizedType.newInstance();
                                    newInstance.setId(id);
                                    ((List)newValue).add(newInstance);
                                } catch (Exception e) {
